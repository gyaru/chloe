...
pub async fn status(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let embed = serenity::CreateEmbed::new()
        ...
        .field("collection time", format!("{}ms", collection_time.as_millis()), true)
        .footer(serenity::CreateEmbedFooter::new("chloe v0.1.0 • made with sparkle and good vibes! ✨"));
...
async fn check_database_health(db_pool: &sqlx::PgPool) -> String {
    ...
    match test_query {
        ...
        Err(e) => format!("**status:** 🔴 error\n**error:** {}", e.to_string().chars().take(50).collect::<String>())
    }
}
...
async fn check_redis_health(redis_client: &redis::Client) -> String {
    ...
    let mut con = match redis_client.get_connection() {
        Ok(con) => con,
        Err(e) => return format!("**status:** 🔴 connection failed\n**error:** {}", e.to_string().chars().take(50).collect::<String>())
    };
